    ...
    <div class="max-w-screen min-h-screen bg-gray-100 text-black">
      ...
      <div class="flex flex-col items-center justify-center min-h-screen selection:bg-[#252850] selection:text-white">
        ...
        <img
          ...
          :src="getImageUrl(props.tasks[currentIndex].image)"     
          :alt="props.tasks[currentIndex].title"                  
          class="mt-4 w-96 h-auto rounded-lg shadow-lg"
        />
        ...
        <div class="w-full max-w-6xl p-6">
          <div class="flex flex-wrap gap-4 justify-between mb-4">
            ...
            <select v-model="selectedCategory" class="px-4 py-2 border rounded-lg text-black">
              <option value="">Todas las categorías</option> <!-- Opción por defecto -->
              <option v-for="category in uniqueCategories" :key="category" :value="category">
                {{ category }}                                <!-- Opciones generadas dinámicamente -->
              </option>
            </select>
          </div>
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-6 py-4 space-x-2">
                    <Link :href="route('task.show', task.id)" class="text-blue-600 hover:underline">Ver</Link>
                    <Link :href="route('task.edit', task.id)" class="text-green-600 hover:underline">Editar</Link>
